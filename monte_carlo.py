...
class MonteCarlo():
    ...
    def __init__(
        ...
        self._init_offset = intial_offset
        self._init_stddev = intial_stddev
        self._offset = offset
        self._stddev = stddev

        # initialize our walkers
        walker_size = (1, batch_size)
        self.walkers = np.zeros(shape=walker_size, dtype=dtype)
    ...
    def compute_psi(self):
        """ x """
        return network.forward(self.walkers)
    ...
    def metropolis_accept_step(self, acceptance_ratio):
        ...
        # test the condition
        accepted = bool(np.log(u) <= acceptance_ratio)
    ...
    def preform_one_step(list_of_states, list_of_ratios, list_of_accepts, dim_of_samples, *args, **kwargs):
        ...
        new_psi = network.forward(new_state)
